...
def estimate_grid_dimensions(n: int) -> tuple[int, int]:
    """Return (n_rows, n_cols) for *n* patches.

    Finds the most-square rectangle whose area equals *n*.  For perfect squares
    (e.g. 225 → 15×15) the result is square.  For non-square counts (e.g.
    221 = 13×17) it returns the factor pair closest to √n so the reconstructed
    map is as compact as possible.  The convention is n_rows ≤ n_cols.
    """
    if n <= 0:
        raise ValueError(f"Cannot compute grid dimensions for {n} patches")
    best_rows = 1
...
def load_patches(patches_dir: Path) -> dict[int, np.ndarray]:
    ...
    first_shape = patches[0].shape
    n_rows, n_cols = estimate_grid_dimensions(len(patches))

    print(f"[load] {len(patches)} patches | shape: {first_shape} | grid: {n_rows}x{n_cols}")
...
def quick_reconstruction_score(patches: dict[int, np.ndarray]) -> float:
    n = len(patches)
    grid_shape = estimate_grid_dimensions(n)
    ...
    for overlap_width in sorted(set(overlap_candidates)):
        ...
        exact_grid = exact_overlap_reconstruct(comp, grid_shape, cfg)
        if exact_grid is not None:
            score = grid_seam_energy(exact_grid, comp)
        else:
            grid = frontier_reconstruct(comp, grid_shape, cfg)
...
def _cell_boundary_bonus(state, row: int, col: int, grid_shape: tuple[int, int], comp: Compatibility, cfg: SolverConfig):
...
def _state_grid(flat: tuple[int, ...], grid_shape: tuple[int, int]) -> list[list[int]]:
    n_rows, n_cols = grid_shape
    return [list(flat[r * n_cols : (r + 1) * n_cols]) for r in range(n_rows)]


def exact_overlap_reconstruct(comp: Compatibility, grid_shape: tuple[int, int], cfg: SolverConfig) -> list[list[tuple[int, int]]] | None:
    ...
    grid: list[list[int | None]] = [[None] * n_cols for _ in range(n_rows)]
    grid[0][0] = 0
    ...
    while changed:
        ...
        for row in range(n_rows):
            for col in range(n_cols):
                ...
                if pid is None:
                    continue
                if col + 1 < n_cols and grid[row][col + 1] is None and len(right_cands[pid]) == 1:
                    grid[row][col + 1] = next(iter(right_cands[pid]))
                    changed = True
                if row + 1 < n_rows and grid[row + 1][col] is None and len(down_cands[pid]) == 1:
    ...
    missing = [(row, col) for row in range(n_rows) for col in range(n_cols) if grid[row][col] is None]
    ...
    def cell_candidates(row: int, col: int) -> set[int]:
        ...
        if col + 1 < n_cols and grid[row][col + 1] is not None:
            cand &= left_of[grid[row][col + 1]]
        if row + 1 < n_rows and grid[row + 1][col] is not None:
    ...
    solved = solutions[0]
    if any(len(row) != n_cols for row in solved) or len(solved) != n_rows:
        return None
...
def _frontier_cell_scores(
    ...
    scores += _cell_boundary_bonus(np.arange(len(comp.states)), row, col, grid_shape, comp, cfg)
    if cfg.rotation_penalty:
        scores += comp.state_penalties
    return scores


def frontier_reconstruct(comp: Compatibility, grid_shape: tuple[int, int], cfg: SolverConfig) -> list[list[tuple[int, int]]]:
    n_rows, n_cols = grid_shape
    ...
    for anchor_state in anchor_states:
        ...
        while frontier:
            ...
            for pos in frontier:
                ...
                if not (0 <= row < n_rows and 0 <= col < n_cols):
                    continue
                scores = _frontier_cell_scores(placements, pos, grid_shape, comp, cfg)
            ...
            for neighbor in ((row, col - 1), (row, col + 1), (row - 1, col), (row + 1, col)):
                nr, nc = neighbor
                if 0 <= nr < n_rows and 0 <= nc < n_cols and neighbor not in placements:
        ...
        energy = _seam_energy(flat, grid_shape, comp)
    ...
    refined = refine_assignment(best_flat, grid_shape, comp, cfg)
    refined = swap_refine_assignment(refined, grid_shape, comp, cfg)
    refined = block_refine_assignment(refined, grid_shape, comp, cfg)
    return [[comp.states[state] for state in row] for row in _state_grid(refined, grid_shape)]


def beam_reconstruct(comp: Compatibility, grid_shape: tuple[int, int], cfg: SolverConfig) -> list[list[tuple[int, int]]]:
    n_rows, n_cols = grid_shape
    anchor_states = [comp.state_index[(0, rot)] for rot in cfg.anchor_rotations if (0, rot) in comp.state_index]
    if not anchor_states:
        raise RuntimeError("No valid anchor states available for patch_0")
    beams: list[tuple[tuple[int, ...], frozenset[int], float]] = [
        ((anchor_state,), frozenset({0}), float(_cell_boundary_bonus(anchor_state, 0, 0, grid_shape, comp, cfg)))
        for anchor_state in anchor_states
    ]
    ...
    for pos in range(1, total_cells):
        ...
        for flat, used, score in beams:
            ...
            for state in _top_states(candidate_score, cfg.beam_candidates):
                pid, _ = comp.states[state]
                step = float(candidate_score[state]) + _cell_boundary_bonus(state, row, col, grid_shape, comp, cfg)
        ...
        if pos % n_cols == n_cols - 1:
            elapsed = time.time() - t0
            print(f"[beam] Row {row + 1:2d}/{n_rows} | {elapsed:.1f}s | best {beams[0][2]:.3f}")
...
def _seam_energy(flat: tuple[int, ...], grid_shape: tuple[int, int], comp: Compatibility, positions: set[int] | None = None) -> float:
    n_rows, n_cols = grid_shape
    energy = 0.0
...
def _affected(pos_a: int, pos_b: int, grid_shape: tuple[int, int]) -> set[int]:
    n_rows, n_cols = grid_shape
    out: set[int] = set()
...
def _local_scores(flat: tuple[int, ...], pos: int, grid_shape: tuple[int, int], comp: Compatibility, cfg: SolverConfig) -> np.ndarray:
    n_rows, n_cols = grid_shape
    row, col = divmod(pos, n_cols)
    scores = np.zeros(len(comp.states), dtype=np.float32)
    if col > 0:
    ...
    if col < n_cols - 1:
        scores += comp.right_cost[:, flat[pos + 1]]
    if row > 0:
        scores += comp.down_cost[flat[pos - n_cols]]
    if row < n_rows - 1:
        scores += comp.down_cost[:, flat[pos + n_cols]]
    scores += _cell_boundary_bonus(np.arange(len(comp.states)), row, col, grid_shape, comp, cfg)
    if cfg.rotation_penalty:
...
def _best_rotation_for_piece(
    ...
    scores = _local_scores(flat, pos, grid_shape, comp, cfg)
    state_ids = comp.piece_to_states[pid]
    return min(state_ids, key=lambda state: float(scores[state]))


def refine_assignment(flat: tuple[int, ...], grid_shape: tuple[int, int], comp: Compatibility, cfg: SolverConfig) -> tuple[int, ...]:
    current = list(flat)
    anchor = comp.state_index[(0, cfg.anchor_rotations[0])]
    print(f"[refine] Initial seam energy: {_seam_energy(tuple(current), grid_shape, comp):.3f}")
    ...
    for pass_id in range(cfg.refine_passes):
        ...
        for pos in range(1, len(current)):
            ...
            best_state = _best_rotation_for_piece(tuple(current), pos, pid, grid_shape, comp, cfg)
        ...
        for pos in range(1, len(current)):
            ...
            scores = _local_scores(before_flat, pos, grid_shape, comp, cfg)
            ...
            for state in _top_states(scores, cfg.refine_candidates):
                ...
                affected = _affected(pos, swap_pos, grid_shape)
                old_energy = _seam_energy(before_flat, grid_shape, comp, affected)
                ...
                for state_a in comp.piece_to_states[candidate_pid]:
                    for state_b in comp.piece_to_states[before_pid]:
                        ...
                        trial_energy = _seam_energy(tuple(trial), grid_shape, comp, affected)
        ...
        energy = _seam_energy(tuple(current), grid_shape, comp)
...
def swap_refine_assignment(
    ...
    current = list(flat)
    print(f"[2opt] Initial seam energy: {_seam_energy(tuple(current), grid_shape, comp):.3f}")
    ...
    for pass_id in range(cfg.swap_refine_passes):
        ...
        for pos_a in range(1, len(current) - 1):
            for pos_b in range(pos_a + 1, len(current)):
                affected = _affected(pos_a, pos_b, grid_shape)
                old_energy = _seam_energy(tuple(current), grid_shape, comp, affected)
                ...
                for state_a in comp.piece_to_states[pid_b]:
                    for state_b in comp.piece_to_states[pid_a]:
                        ...
                        new_energy = _seam_energy(tuple(current), grid_shape, comp, affected)
        ...
        total = _seam_energy(tuple(current), grid_shape, comp)
        print(
...
def _window_positions(top: int, left: int, grid_shape: tuple[int, int]) -> tuple[list[int], set[int]]:
    n_rows, n_cols = grid_shape
    cells = []
    affected: set[int] = set()
    for dr in (0, 1):
        for dc in (0, 1):
            pos = (top + dr) * n_cols + (left + dc)
            cells.append(pos)
            row, col = divmod(pos, n_cols)
            affected.add(pos)
            if col > 0:
...
def block_refine_assignment(
    ...
    n_rows, n_cols = grid_shape
    if cfg.block_refine_passes <= 0 or n_rows < 2 or n_cols < 2:
        return flat

    current = list(flat)
    print(f"[block] Initial seam energy: {_seam_energy(tuple(current), grid_shape, comp):.3f}")

    for pass_id in range(cfg.block_refine_passes):
        changes = 0
        for top in range(n_rows - 1):
            for left in range(n_cols - 1):
                cells, affected = _window_positions(top, left, grid_shape)
                base_flat = tuple(current)
                base_energy = _seam_energy(base_flat, grid_shape, comp, affected)
...
def reconstruct_grid(patches: dict[int, np.ndarray], grid_shape: tuple[int, int], cfg: SolverConfig | None = None) -> list[list[tuple[int, int]]]:
    cfg = cfg or SolverConfig()
    comp = build_compatibility(patches, cfg)
    exact_grid = exact_overlap_reconstruct(comp, grid_shape, cfg)
    if exact_grid is not None:
        return exact_grid
    return frontier_reconstruct(comp, grid_shape, cfg)
...
def grid_seam_energy(grid: list[list[tuple[int, int]]], comp: Compatibility) -> float:
    ...
    return _seam_energy(flat, (n_rows, n_cols), comp)
...
def stitch_map(
    ...
    canvas_w = patch_w + (grid_cols - 1) * stride_x
    canvas_h = patch_h + (grid_rows - 1) * stride_y
