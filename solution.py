...
def choose_reconstruction_config(
    ...
    for overlap_width in overlap_candidates:
        ...
        score, seam_energy, rotated_tiles = candidate_score(patches, grid_shape, candidate_cfg)
        print(
...
def main() -> None:
    ...
    if args.use_existing_map:
        ...
        print(f"[config] {n} patches -> grid {n_rows}x{n_cols}")
        ...
        if args.fixed_config:
            ...
            solver_cfg = choose_reconstruction_config(patches, grid_shape, selected_mode, patches_dir)

        grid = reconstruct_grid(patches, grid_shape, solver_cfg)
